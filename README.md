...
INFO.txt + atlas.npy
  ...
  -> grey_white_split.py        Этап 4 (опц.): мозг -> серое (оболочка) + белое (ядро) эрозией
  -> 02_merged.npy
  -> режим сеток:
...
### Серое/белое вещество (синтез эрозией)

Если атлас не содержит разметки серого/белого (как Digimouse — там только
анатомические под-структуры мозга, которые при вложении пересекаются), можно
синтезировать чистую пару оболочек: **цельный мозг делится эрозией на серую
оболочку (снаружи) и белое ядро (внутри)**. Граница геометрическая, а не
гистологическая, но для переноса света важны именно разные оптические свойства
слоёв — их это и даёт, с настраиваемой толщиной серого. Результат — строго
вложенная модель `кожа ⊃ череп ⊃ серое ⊃ белое` без дыр и без пересечений.

Опция включается в конфиге (или галкой «Разделить мозг на серое/белое» в GUI,
режим "Оболочки", с полем толщины серого в мм). Шаг `grey_white_split.py`
запускается после сглаживания, перед `build_envelopes`. Ключи конфига:

- `grey_white_split` — включить шаг;
- `gw_brain_labels` — метки в `02_merged`, образующие цельный мозг;
- `gw_grey_mm` — толщина серой оболочки (мм; по умолчанию 0.6);
- `gw_white_label` / `gw_grey_label` — метки ядра и оболочки;
- в пару к ним: `envelope_parents` с вложенностью серое⊃белое и
  `label_names_extra` с именами обеих меток.

Готовый пример — [configs/pipeline_config_mouse_gw.json](configs/pipeline_config_mouse_gw.json)
(отдельный `output_dir`, чтобы не смешивать с обычной моделью мыши).
