...
слоёв — их это и даёт, с настраиваемым разбиением. Результат — строго
вложенная модель `кожа ⊃ череп ⊃ серое ⊃ белое` без дыр и без пересечений.

Разбиение задаётся одним из двух способов:

- **толщина серого в мм** (`gw_grey_mm`) — эрозия мозга на столько мм даёт белое
  ядро, остаток — серая оболочка;
- **доля объёма** (`gw_grey_fraction`, 0–1; например `0.2` = серое 1/5, белое
  4/5) — внешняя доля мозга по объёму становится серым, глубже — белым (через
  порог по distance-transform, точная доля независимо от формы).

Опция включается в конфиге (или галкой «Разделить мозг на серое/белое» в GUI,
режим "Оболочки"; там же переключатель «толщина (мм) / доля объёма»). Шаг
`grey_white_split.py` запускается после сглаживания, перед `build_envelopes`.
Ключи конфига:
...
- `gw_grey_fraction` — доля серого по объёму (0–1); если задана, имеет приоритет
  над `gw_grey_mm`;
...
Готовые примеры — [configs/pipeline_config_mouse_gw.json](configs/pipeline_config_mouse_gw.json)
(по толщине) и [configs/pipeline_config_mouse_gw15.json](configs/pipeline_config_mouse_gw15.json)
(серое = 1/5 объёма); у каждого отдельный `output_dir`, чтобы не смешивать
модели.

## Метрики поверхностей

`surface_metrics.py` (галка «Считать метрики» в GUI или запуск с `--config`)
считает по каждой оболочке набор метрик и пишет `metrics.{json,csv,txt}` в папку
поверхностей; в GUI они же показываются во вкладке «Метрики». Расшифровка колонок:

**Геометрия**

| колонка | что это | норма |
|---|---|---|
| `ткань` | имя ткани (`surface_NN_<ткань>`) | — |
| `граней` | число треугольников поверхности | — |
| `площадь см²` | площадь поверхности (в коде мм², в таблице см²) | — |
| `объём см³` | объём, заключённый внутри оболочки (см³) | — |

**Качество и топология** — годность сетки для MC:

| колонка | что это | норма |
|---|---|---|
| `wt` | watertight: нет граничных и non-manifold рёбер (замкнутая ориентируемая поверхность) | `Y` — обязательно для MC |
| `genus` | топологический род = число сквозных «ручек»/тоннелей (0 = как сфера) | 0 для мозга/кожи; у черепа >0 — это анатомические форамины/швы, нормально |
| `комп` | число связных компонент | 1 (одна оболочка) |
| `self-x` | число самопересекающихся граней | 0 |
| `asp99` | 99-й перцентиль aspect ratio треугольников (`circumradius/(2·inradius)`; 1 = равносторонний, большой = «игла») | ближе к ~2 = ровная сетка |
| `sliver%` | доля треугольников с минимальным углом < 10° (тонкие «щепки») | чем меньше, тем лучше |

**Точность (fidelity)** — насколько поверхность верна исходнику:

| колонка | что это | норма |
|---|---|---|
| `RMSvox` | RMS-отклонение вершин (мм) от **воксельной границы оболочки** (эталон — знаковый EDT региона, восстановленного из вокселей) | порядка размера вокселя = отлично |
| `MAXvox` | максимальное такое отклонение (мм) | — |
| `RMSraw` | RMS-отклонение (мм) от **сырой CGAL-сетки** (до Taubin/децимации) — сколько «съели» сглаживание и децимация | малое = чистка не исказила форму |
| `Hraw` | симметричное расстояние **Хаусдорфа** (мм) до сырой CGAL-сетки (худшее двустороннее отклонение) | — |

**Вложенность** — корректность для surface-MC:

| колонка | что это | норма |
|---|---|---|
| `minGap` | минимальный зазор (мм) до **родительской** оболочки (`>0` = оболочка внутри родителя на столько мм); у корня `n/a` | больше = безопаснее |
| `poke%` | доля вершин, оказавшихся **снаружи** родителя (торчание сквозь него) | **0.000** — иначе оболочки пересекаются и surface-MC ломается |

`RMSraw`/`Hraw` считаются только если сборка сохранила сырые сетки (`--keep-work`;
GUI передаёт его автоматически, когда включены метрики), иначе там `n/a`.
