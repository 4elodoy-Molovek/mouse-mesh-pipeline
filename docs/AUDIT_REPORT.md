...
**LOW/S** (запах). OK = исправлено на этой ветке, (!) = рекомендация (§5).

| # | Severity | Файл | Проблема | Статус |
|---|----------|------|----------|--------|
| B1 | HIGH | `npy2conformal_mesh.py` | Tier A: `del … smoother …` даёт `NameError`, если сглаживание выключено (`--smooth 0`) — `smoother` объявлен только внутри `if`. | OK |
| B2 | **BLOCKER** | `npy2conformal_mesh.py` | Taubin-сглаживание тет-сетки выворачивает тетраэдры. **Воспроизведено в этом аудите**: чистая мышь 38 873 тетов, 20 итераций старого сглаживания → **222 инвертированных**; на поставленной сетке мыши — **286**. Отрицательный якобиан = неопределённое «внутри/снаружи» = блокер трассировки фотонов. | OK |
| B3 | MED | `small_area_closer.py` | Мажоритарное восстановление голосует по **несмёрженному** `original_volume` → слитая/занулённая метка может вернуться в итог. Латентно на текущей мыши (cerebrum проголосовал за 7), но проявляется, когда неканоническая метка доминирует в компоненте. | OK |
| B4 | HIGH | `connectivity_searcher.py`, `npy2conformal_mesh.py` | `UnicodeEncodeError` при выводе `→`/`─`/`↳` в лог на не-UTF-8 кодовой странице Windows (cp1251/cp1252) при прямом запуске (не из GUI, где задан `PYTHONIOENCODING`). | OK |
| B5 | LOW | `connectivity_searcher.py` | Лог обрезки по Y умножал Y-индекс на `dz` вместо `dy` (неверный мм-экстент при анизотропии; на мыши spacing изотропен → косметика). | OK |
...
| B7 | MED | `npy2conformal_mesh.py` `validate_conformal_surfaces` | Вердикт «PASSED / ВСЕ СЕТКИ OK» опирается **только** на watertight, игнорируя genus и MaxErr → сетки со 143 мм ошибки помечены OK. | (!) R2 (частично закрыто `mc_mesh_check.py`) |
| B8 | MED (операционный) | выходные `.mesh` | **Поставленная сетка мыши устарела и не соответствует текущему пайплайну.** На диске материалы `{1,2,3,4,5,6,7,8}` (`eye`, `label_8`), а текущий `02_merged.npy` = `{1,2,4,5,6,7,11,12}`. Плюс 286 инверсий (создана до фикса B2). **Нужна перегенерация.** | (!) R0 |
| S1 | S | `npy2conformal_mesh.py` | `if was_decimated or True:` — мёртвая ветка (всегда истина). | (!) R6 |
| S2 | S | `connectivity_searcher.py` | `recode_labels`, `_RECODE_MAP`, `LABEL_NAMES` не используются в `__main__`. | (!) R6 |
| S3 | MED | `meshValidator.py` | `HIERARCHY = {3:100, 6:10}` захардкожен под метки CRISP (мозг/кожа); для мыши id иные → приоритеты CSG неверны. | (!) R6 |
| S4 | MED | конфиги | `pipeline_config.json` (по README — «brain») указывает на данные **мыши**; CRISP `INFO.txt` в формате MINC, который `parse_info_txt` не разбирает → путь мозга держится на config-fallback. | (!) R4 |
| S5 | MED | `small_area_closer.py` | `min_voxels=10000` удаляет мелкие ткани целиком. На мыши исчезают **eye (3)** и **adrenal glands (20)** — теряют собственные оптические свойства в MC. | (!) R5 |
| S6 | LOW | `parse_info_txt.py` | Если одно имя класса встречается на нескольких строках с немонотонными canonical-метками (новая меньше существующей), в `label_names` появляются два ключа с одним именем, а старые merges не переуказываются. На текущих INFO.txt не проявляется. | (!) R6 |
...
- **Connected components / relabel** (`relabel_existing_labels`, connectivity=3 = 26-связность):
  ...
  нумерация сквозная (offset `next_label-1` на компоненту). OK
- **Слияния меток из `INFO.txt`**: `canonical = min`, фоновые `{N:0}` реально зануляются.
  ...
  компаундные строки с `+` («whole brain») пропускаются; `02_merged` не содержит 8 и 10. OK
...
- **Taubin `_smooth_tet_points`**: λ=0.5, μ=−0.53, |μ|>λ (анти-усадка) — формула верна;
  ...
  **Эмпирическая проверка (этот аудит), чистая мышь, 20 итераций:**
  ...
  вершин, макс. смещение остальных 0.75 мм — сглаживание сохранено). OK
- **Извлечение поверхностей из тетов** (`_extract_region_surface`): граничная грань = грань
  ...
  Ориентация из сырого winding чинится `fix_normals`. OK
  (!) Важно: **пер-тканевые VTK после `validate_conformal_surfaces` теряют взаимную
...
|----------|------------------------------------------|-----------------------------------------|
| Положительный якобиан | OK **0 / 2 670 156** инвертированных | FAIL **286 / 38 873** (Taubin) — чинится B2 + перегенерацией |
| Конформность (грань ≤ 2 тетов) | OK 0 нарушений; 40 940 граничных граней | OK 0 нарушений; 5 684 граничных граней |
| Герметичность региона (нет дыр) | OK все регионы замкнуты | OK все регионы замкнуты |
| Многообразность границ | (!) non-manifold рёбра на тройных стыках тканей — **норма** для мультиматериала | (!) то же (skin 560, skeleton 418, olfactory 15) |
| Качество (слайверы q<0.01) | OK 0 (q_min 0.0102) | (!) 5 слайверов (q_min 0.0010) |
| Единицы / масштаб | OK мм, bbox 142×216×180 | OK мм, bbox 16×30×28 |
| Разметка материалов | OK 10 регионов, region→ткань | (!) 8 регионов, но **устаревшие** (B8) |
