...
class PipelineApp:
    ...
    def build_ui(self) -> None:
        ...
        rb_classic = tk.Radiobutton(
            f_mode,
            text="Классический: npy2vtk → meshValidator  (быстро, независимые изоповерхности)",
        ...
        rb_cgal = tk.Radiobutton(
            f_mode,
            text="Конформный CGAL (pygalmesh): единая тет-сетка + VTK + валидация  (медленно)",
            variable=self.mesh_mode_var,
            value="cgal",
            command=self._on_mode_change,
            fg="darkblue",
        ...
        rb_env = tk.Radiobutton(
            f_mode,
            text="Оболочки (build_envelopes): вложенные внешние оболочки тканей — ПРАВИЛЬНЫЙ вход для вашего surface-MC",
            variable=self.mesh_mode_var,
            value="envelopes",
            command=self._on_mode_change,
            fg="#8000a0",
